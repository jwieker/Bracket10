...
*   **Ranking Simulation:** `possibleRanking` simulates best-case scenarios. `minPoints` guarantees a floor by analyzing pick paths that clash in future games.
    *   **Cascading Clash Propagation Algorithm**: Rather than truncating pick paths at the first clash (which undercounts guaranteed points in downstream rounds), the algorithm evaluates game outcomes round-by-round from the bottom up. In each round, if multiple picks from the same entry reach a game, the algorithm selects the one that guarantees the maximum potential points (the "floor") to advance to the next round, ensuring cascading clash points (e.g. winner of A/B playing winner of C/D) are correctly and exactly accounted for.
    *   **Fallback**: To preserve backward compatibility with legacy tests where full game graphs are not supplied, the algorithm falls back to simple first-clash truncation if game structure arguments (`sortedGames`, `incomingGames`) are omitted.
    *   `getHighestPlace` compares an entry's max potential against every other entry's guaranteed minimum.
...
*   **Undoing:** `undoGame` must cleanly roll back state. If undoing a Round 1 game where the team had already advanced and played in Round 2, the system must cascade the deletion without corrupting the rest of the bracket. Every undo clears the winner **and sets `manualHold: true` in the same write** (`clearWinnerWithHold`) so the ESPN poll — whose feed still lists the game as completed for ~48h — cannot re-apply the undone result. The hold releases when a result is recorded or via `POST /releaseGameHold`.
*   **First Four commit-marker ordering:** Round-0 resolution writes the `winner` field LAST (after the R1 slot fill, the entry pick auto-swap, and the canonical school record). The winner field is the poll's retry gate (`winner == null` filter), so a mid-resolution failure leaves the game unresolved and the next poll cycle redoes every idempotent step — a crash can never permanently strand combined picks. The undo path mirrors this: pick repick → canonical-record delete → slot clear → winner cleared last (a half-failed undo leaves the game resolved; the admin simply retries).
*   **FF undo repick:** Undoing a First Four game normalizes every affected entry's pick back to `team1ID` (the combined "A / B" option's value) regardless of which team had won. If the caller omits `team1ID`, the service derives it from the round-0 game doc.
*   **Write-time FF pick normalization (`normalizeFirstFourPicks`, viewService):** Entry creation and all edit flows map any FF-team pick against LIVE round-0 game state before validation — unresolved game → `team1ID`, resolved game → the winner. Whoever the user picks, they get the winner, regardless of form/cache staleness; a stranded loser pick is healed on the entry's next edit. Runs before the uniqueness check so both FF teams of one game submitted together are rejected as duplicates.
