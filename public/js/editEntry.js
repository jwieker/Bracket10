...
function updateBracketVisual() {
  ...
  $('select.form-selector').each(function () {
    ...
    if (
      ...
      selectedOption.data('seed') !== undefined
    ) {
      ...
      if (regionArrayIndex !== -1) {
        ...
        const elementsToPick = document.querySelectorAll(
          `.team[data-region="${targetDataRegionValue}"][data-seed="${seed}"]`,
        );
        elementsToPick.forEach((el) => el.classList.add('picked'));
      } else {
        console.warn(
          `Region name "${regionName}" not found in regionData. Cannot highlight team in static bracket.`,
        );
      }
    }
  });
...
function adjustR3MatchupMargins() {
  const roundR3Elements = document.querySelectorAll(
    '.bracket-wrapper .round.r3',
  );
  ...
  const roundR4Elements = document.querySelectorAll(
    '.bracket-wrapper .round.r4',
  );
...
function adjustTeamNameFontSize() {
  const teamNameSpans = document.querySelectorAll(
    '#bracketRow .round.r1 .team span.name',
  );
...
async function fetchAndUpdateMaxPoints(selectedTeamValues) {
  ...
  const selectedTeamSIDs = selectedTeamValues.map((value) =>
    value.split(',')[0].trim(),
  );
  // The hidden `year` input carries the entry's own year (which may be a past,
  // archived tournament when an admin opens /viewEntry?...&year=<past>). Without
  // it, the server defaults to the current year and the preview silently shows
  // wrong totals for past-year sIDs looked up against the wrong year's data (#376).
  ...
  try {
    const response = await fetch('/calculateMaxPoints', {
      ...
      body: JSON.stringify({ teamSIDs: selectedTeamSIDs, year }),
    });
    ...
    if (maxPointsData.data.maxPoints) {
      ...
      console.warn(
        'Received invalid data format for max points:',
        maxPointsData,
      );
    ...
    $('select.form-selector').each(function (index) {
      ...
      if (index < entryPicksFromServer.length) {
        ...
        if (foundOptionValue) {
          ...
          console.warn(
            `Dropdown ${selectElement.attr('id')}: No option found for sID ${targetSID}. Pick index: ${index}.`,
          );
        }
      }
    });
      ...
      $(this)
        ...
        .each(function () {
          ...
          if (optionValue && optionValue !== '') {
            if (
              selectedTeamsMap.has(optionValue) &&
              selectedTeamsMap.get(optionValue) !== currentSelect
            ) {
...
$(document).ready(function () {
  ...
  const r1TeamSlots = document.querySelectorAll(
    '.bracket-wrapper .team[data-round="1"]',
  );
  r1TeamSlots.forEach((slot) => {
    ...
    if (dataRegion >= 1 && dataRegion <= regionData.length) {
      ...
      const teamInfo = teamData.find(
        (t) => t.seed === seed && t.regionName === regionName,
      );
  ...
  $('form').submit(function (event) {
    ...
    if (selectedTeams.size !== 10) {
      alert(
        'Please pick 10 teams. If this is an error report it to your group admin',
      );
      return false;
    }
