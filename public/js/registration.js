...
function updateConferenceStatsUI() {
  ...
  // Re-render badges
  const sortedConferences = Object.entries(conferenceStatsData).sort(
...
function updateBracketVisual() {
  ...
  allStaticTeams.forEach((teamEl) =>
    teamEl.classList.remove('picked-win', 'picked-loss', 'picked'),
  );
...
async function fetchAndUpdateMaxPoints(selectedTeamValues) {
  ...
  try {
    ...
    if (maxPointsData.data.maxPoints) {
      ...
      document.getElementById('maxPointsHidden').value =
        maxPointsData.data.maxPoints;
    } else {
      console.warn(
...
$(document).ready(function () {
  ...
  function updateNamePreview() {
    ...
    if (!name && !team) {
      ...
      const displayText =
        fullText.length > MAX_NAME_DISPLAY
          ? fullText.substring(0, MAX_NAME_DISPLAY) + '...'
          : fullText;
