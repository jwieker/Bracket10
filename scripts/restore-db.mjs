...
if (!projectId && !process.env.FIRESTORE_EMULATOR_HOST) {
  console.error(
    'Error: GCP_PROJECT_ID environment variable is required to run this script (unless FIRESTORE_EMULATOR_HOST is set).',
  );
  process.exit(1);
}
...
async function restoreCollection(collection) {
  ...
  if (!backupFilePath) {
    console.log(
      `  ⚠ No backup files found for collection "${collection}". Skipping.`,
    );
    return;
  }
  ...
  for (const line of lines) {
    ...
    try {
      ...
      console.error(
        `  ✗ Error parsing JSON line: "${line.substring(0, 100)}..."`,
        err,
      );
      continue;
    }
    ...
    if (!_id) {
      console.error(
        `  ✗ Skipping document: missing "_id" property in backup line.`,
      );
      continue;
    }
  ...
  if (isDryRun) {
    console.log(
      `  [DRY RUN] Would have written ${lines.length} document(s) to collection "${collection}".`,
    );
  } else {
    console.log(
      `  ✅ Successfully restored ${totalWritten} document(s) to collection "${collection}".`,
    );
  }
}
