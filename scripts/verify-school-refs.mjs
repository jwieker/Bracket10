...
async function main() {
  ...
  for (const year of years) {
    ...
    const recordsSnap = await db
      .collection(`tournaments/${year}/schoolRecords`)
      .get();
    ...
    const entriesSnap = await db
      .collection(`tournaments/${year}/entries`)
      .get();
  ...
  if (recordIssues.length === 0) {
    ...
    for (const issue of recordIssues) {
      console.log(
        `  tournaments/${issue.year}/schoolRecords/${issue.docId} → sID "${issue.sID}"`,
      );
    }
  }
  ...
  if (entryIssues.length === 0) {
    ...
    for (const issue of entryIssues) {
      ...
      if (!byYear[key])
        byYear[key] = { year: issue.year, sID: issue.sID, count: 0 };
      byYear[key].count++;
    }
    for (const { year, sID, count } of Object.values(byYear)) {
      console.log(
        `  [${year}] sID "${sID}" missing from school collection (appears in ${count} entries)`,
      );
    }
    console.log(
