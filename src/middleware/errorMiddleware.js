...
export function errorMiddleware(err, req, res, next) {
    try {
        ...
        if (err instanceof ValidationError) {
            Logger.warn(`Validation error at ${req.method} ${req.originalUrl}`, { field: err.field, message: err.message });
            const payload = { error: "Validation Error", message: err.message, field: err.field };
            return isJson ? res.status(400).json(payload) : res.status(400).send(payload.message);
        }

        if (err instanceof DatabaseError) {
            Logger.error(`Database error at ${req.method} ${req.originalUrl}`, err);
            const payload = debugErrorsEnabled()
                ? { error: "Database Error", message: err.message, operation: err.operation }
                : { error: "Database Error", message: "A database error occurred." };
            return isJson ? res.status(500).json(payload) : res.status(500).send("A database error occurred.");
        }
        ...
        if (err instanceof ServiceError) {
            ...
            return isJson ? res.status(500).json(payload) : res.status(500).send("A service error occurred.");
        }

        // Unknown error
        Logger.error(`Unhandled error at ${req.method} ${req.originalUrl}`, err);
        const payload = { error: "Internal Server Error", message: "An unexpected error occurred" };
        return isJson ? res.status(500).json(payload) : res.status(500).send(payload.message);
