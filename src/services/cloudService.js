...
async function getMonthToDateSpend() {
  ...
  if (!projectId) {
    return {
      ...
      currency: 'USD',
      error:
        'GCP project ID could not be determined. Please configure GOOGLE_CLOUD_PROJECT or GCP_PROJECT_ID.',
    };
  }
  ...
  const runQuery = async (withCredits) => {
    ...
    // jobComplete: false (query still running past BigQuery's ~10s default wait)
    // also arrives with no `rows` — without this check it fell through to the
    // "no rows" branch below and rendered as a reassuring $0 instead of an
    // unknown/error state on the page that exists to catch real overspend.
...
async function getDailySpend() {
  ...
  const runQuery = async (withCredits) => {
    ...
    // Same jobComplete guard as getMonthToDateSpend — an incomplete query has
    // no `rows` and must not be read as a genuinely empty (zero-spend) month.
    if (data.jobComplete === false) {
      throw new Error('BigQuery query did not complete in time');
    }
...
export async function getCloudConsoleLinks() {
  ...
  return {
    ...
    firestore: projectId
      ? `https://console.cloud.google.com/firestore/databases/-default-/data?project=${enc}`
      : '#',
    cloudRun: projectId
      ? `https://console.cloud.google.com/run?project=${enc}`
      : '#',
    cloudBuild: projectId
      ? `https://console.cloud.google.com/cloud-build/builds?project=${enc}`
      : '#',
    cloudBuildTriggers: projectId
      ? `https://console.cloud.google.com/cloud-build/triggers?project=${enc}`
      : '#',
    logs: projectId
      ? `https://console.cloud.google.com/logs/query?project=${enc}`
      : '#',
    billing: projectId
      ? `https://console.cloud.google.com/billing?project=${enc}`
      : '#',
    budgets: projectId
      ? `https://console.cloud.google.com/billing/budgets?project=${enc}`
      : '#',
    artifactRegistry: projectId
      ? `https://console.cloud.google.com/artifacts?project=${enc}`
      : '#',
    iam: projectId
      ? `https://console.cloud.google.com/iam-admin/iam?project=${enc}`
      : '#',
    secrets: projectId
      ? `https://console.cloud.google.com/security/secret-manager?project=${enc}`
      : '#',
  };
}
