...
import { firestoreRateLimit } from "../middleware/rateLimit.js";
import {
  updateTotalPoints,
  getPossibleRanking,
  adminLogin,
...
// not per-process (M1). Keyed by client IP.
...
router.post("/user/logout", requireUser, userLogout);

// Admin-only actions (must come from within the admin console)
router.post("/updateTotalPoints", requireSiteAdmin, verifyCsrf, updateTotalPoints);
...
router.post("/clearGoogleSessions", requireSiteAdmin, verifyCsrf, clearGoogleSessionsHandler);
