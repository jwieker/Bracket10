...
vi.mock("../src/services/tourneyService.js", async () => {
  ...
  return {
    ...
    prepareRegionVerifyData: vi.fn(async (regions, year) => {
...
describe("createNewBracket", () => {
  test("should correctly format games and team records", async () => {
    ...
    expect(gamesFormat[8]).toEqual([16, 1, 2023, 73, 304, null, 1, 24, 1, 1, 16]);
    expect(gamesFormat[16]).toEqual([31, 4, 2023, 41, 262, null, 1, 39, 1, 1, 16]);
    expect(gamesFormat[24]).toEqual([46, 2, 2023, 71, 200, null, 1, 54, 1, 1, 16]);

    // Test future games format (games without teams)
    expect(gamesFormat[32]).toEqual([9, 3, 2023, null, null, null, 2, 13, 1, null, null]);
    expect(gamesFormat[33]).toEqual([10, 3, 2023, null, null, null, 2, 13, 2, null, null]);
...
describe("regionVerify", () => {
  ...
  test("should render newTourneyGames with correct data", async () => {
    ...
    expect(prepareRegionVerifyData).toHaveBeenCalledWith([
      2, 3, 4, 1,
    ], 2026);
