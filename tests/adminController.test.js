...
vi.mock('../src/services/index.js', () => ({
  ...
  getCloudConsoleLinks: vi.fn(() => ({ projectId: 'test-proj', deployBranch: 'main', firestore: 'https://example/fs' })),
}));
...
describe('adminTournamentPage', () => {
  test('renders adminTournament with enhanced games when active games exist', async () => {
    const activeGames = [
      { gameID: 1, winner: 10, team1ID: 10, team1Name: 'Duke', team2Name: 'UNC' },
      { gameID: 2, winner: null, team1ID: 5, team1Name: 'Kansas', team2Name: 'UCLA' },
    ];
    ...
    expect(res.render).toHaveBeenCalledWith('adminTournament', expect.objectContaining({
  ...
  test('sets winner name to team2Name when winner !== team1ID', async () => {
    const activeGames = [
      { gameID: 1, winner: 20, team1ID: 10, team1Name: 'Duke', team2Name: 'UNC' },
    ];
    gameRepository.getActiveGames.mockResolvedValue(activeGames);
...
describe('adminEntriesPage', () => {
  test('renders adminEntries with year', async () => {
    ...
    expect(res.render).toHaveBeenCalledWith('adminEntries', expect.objectContaining({ year: 2024 }));
  });
});
...
describe('adminTeamsPage', () => {
  test('renders adminTeams with year', async () => {
    ...
    expect(res.render).toHaveBeenCalledWith('adminTeams', expect.objectContaining({ year: 2022 }));
  });
});
...
describe('adminSystemPage', () => {
  test('renders adminSystem with year', async () => {
    ...
    expect(res.render).toHaveBeenCalledWith('adminSystem', expect.objectContaining({ year: 2023 }));
  });
});
...
describe('adminCloudPage', () => {
  test('renders adminCloud with budget and links', async () => {
    ...
    expect(res.render).toHaveBeenCalledWith('adminCloud', expect.objectContaining({
      year: 2026,
...
describe('adminCloudBudgetRefresh', () => {
  test('returns budget JSON with force refresh', async () => {
    getBudgetStatus.mockResolvedValue({ configured: true, budgets: [{ displayName: 'B', amount: 5, currency: 'USD' }] });
    const res = mockRes();
    await adminCloudBudgetRefresh(mockReq(), res);
    expect(getBudgetStatus).toHaveBeenCalledWith({ force: true });
    expect(res.json).toHaveBeenCalledWith({ configured: true, budgets: [{ displayName: 'B', amount: 5, currency: 'USD' }] });
  });
});

describe('adminCloudDeploy', () => {
  test('returns 200 with build info on success', async () => {
    triggerProductionDeploy.mockResolvedValue({ ok: true, buildId: 'abc', branch: 'main' });
    const res = mockRes();
    await adminCloudDeploy(mockReq({ method: 'POST' }), res);
    expect(res.status).not.toHaveBeenCalled();
    expect(res.json).toHaveBeenCalledWith({ ok: true, buildId: 'abc', branch: 'main' });
  });
