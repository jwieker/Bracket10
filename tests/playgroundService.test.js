import { buildPlaygroundData, setRepositories } from '../src/services/playgroundService.js';
...
describe('playgroundService', () => {
  ...
  test('buildPlaygroundData prepares full payload, excludes FF losers + FF docs, enriches pending games', async () => {
    const mockDetails = {
      allGames: [
        ...
        { gameID: 64, round: 0, team1ID: 9901, team2ID: 9902, winner: 9901, nextGameID: 5, nextGameSpot: 1 },
        // Pending Round-1 game
        { gameID: 5, round: 1, team1ID: 9901, team2ID: 102, winner: null, nextGameID: 33, nextGameSpot: 2 },
        // Decided Round-1 game
        { gameID: 6, round: 1, team1ID: 103, team2ID: 104, winner: 103, nextGameID: 33, nextGameSpot: 1 },
      ],
      teams: [
        { sID: 9901, name: 'First Four A',     nameNick: 'FFA',         seed: 16, gameStatus: ['W'], isFFDoc: false },
        { sID: 9902, name: 'First Four B',     nameNick: 'FFB',         seed: 16, gameStatus: ['L'], isFFDoc: false },
        { sID: 102,  name: 'Kansas',           nameNick: 'Jayhawks',    seed: 1,  gameStatus: [],    isFFDoc: false },
        { sID: 103,  name: 'Duke',             nameNick: 'Blue Devils', seed: 2,  gameStatus: ['W'], isFFDoc: false },
        { sID: 104,  name: 'North Carolina',   nameNick: 'Tar Heels',   seed: 15, gameStatus: ['L'], isFFDoc: false },
        { sID: 105,  name: 'FF Doc',           nameNick: 'FFDoc',       seed: 16, gameStatus: [],    isFFDoc: true },
      ],
    };

    const mockGroupData = [
      { id: 1, person: 'Family', teamName: 'Bob Pick', picks: [9901, 103], totalPoints: 10, rank: 1, teamsRemaining: 2, otherField: 'should be stripped' },
    ];
    ...
    expect(mockGameRepository.getAllTournamentDetails).toHaveBeenCalledWith(2024);
    expect(buildFullGridDataMock).toHaveBeenCalledWith('BobGroup', 2024, mockDetails);

    // entries: slim shape, extra fields dropped
    expect(result.entries).toEqual([
      { id: 1, person: 'Family', teamName: 'Bob Pick', picks: [9901, 103], totalPoints: 10, rank: 1, teamsRemaining: 2 },
    ]);
    ...
    expect(result.schoolRecords).toEqual([
      { sID: 9901, nameNick: 'FFA',         name: 'First Four A',   seed: 16, gameStatus: ['W'] },
      { sID: 102,  nameNick: 'Jayhawks',    name: 'Kansas',         seed: 1,  gameStatus: [] },
      { sID: 103,  nameNick: 'Blue Devils', name: 'Duke',           seed: 2,  gameStatus: ['W'] },
      { sID: 104,  nameNick: 'Tar Heels',   name: 'North Carolina', seed: 15, gameStatus: ['L'] },
    ]);

    // pendingGames: only round>0 with no winner; enriched with team names + seeds
    expect(result.pendingGames).toEqual([
      {
        gameID: 5, round: 1, roundName: 'First Round',
        nextGameID: 33, nextGameSpot: 2,
        team1ID: 9901, team1Name: 'FFA',      team1Seed: 16,
        team2ID: 102,  team2Name: 'Jayhawks', team2Seed: 1,
      },
    ]);

    // allGamesForClient: excludes round 0, only basic fields
    expect(result.allGamesForClient).toEqual([
      { gameID: 5, round: 1, nextGameID: 33, nextGameSpot: 2, team1ID: 9901, team2ID: 102, winner: null },
      { gameID: 6, round: 1, nextGameID: 33, nextGameSpot: 1, team1ID: 103, team2ID: 104, winner: 103 },
    ]);
    ...
    // roundPoints + roundNames are parallel-indexed
    expect(Array.isArray(result.roundPoints)).toBe(true);
    expect(result.roundPoints.length).toBeGreaterThan(0);
  ...
  test('pendingGames sort by round ascending, then by gameID', async () => {
    const teams = [
      { sID: 1, name: 'A', nameNick: 'A', seed: 1, gameStatus: [], isFFDoc: false },
      { sID: 2, name: 'B', nameNick: 'B', seed: 2, gameStatus: [], isFFDoc: false },
    ];
  ...
  test('repository errors propagate (no swallowing)', async () => {
    mockGameRepository.getAllTournamentDetails.mockRejectedValue(new Error('Firestore unavailable'));
    buildFullGridDataMock.mockResolvedValue({ groupData: [] });
    await expect(buildPlaygroundData('G', 2024)).rejects.toThrow('Firestore unavailable');
  });

  test('with no FF play-in games, all non-FFDoc teams remain in schoolRecords', async () => {
    mockGameRepository.getAllTournamentDetails.mockResolvedValue({
      allGames: [
        { gameID: 1, round: 1, team1ID: 1, team2ID: 2, winner: 1 },
      ],
      teams: [
        { sID: 1, name: 'A', nameNick: 'A', seed: 1, gameStatus: ['W'], isFFDoc: false },
        { sID: 2, name: 'B', nameNick: 'B', seed: 2, gameStatus: ['L'], isFFDoc: false },
      ],
    });
    buildFullGridDataMock.mockResolvedValue({ groupData: [] });
