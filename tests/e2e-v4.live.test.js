...
async function guardedPreClean() {
  ...
  const crudChecks = [
    {
      ref: db.collection('conferences').doc(CRUD_CONF_SLUG),
      ours: (d) => String(d.name || '').includes(MARKER),
    },
    {
      ref: db.collection('school').doc(String(CRUD_SCHOOL_SID)),
      ours: (d) => String(d.name || '').includes(MARKER),
    },
  ...
  if (leftovers.length > 0) {
    logStep('Pre-clean removing leftovers from a prior run', {
...
async function verifyRegistryGone() {
  ...
  for (const ref of registry.values()) {
    ...
    if (snap.exists) {
      throw new Error(
        `[E2E-V4] Teardown verification failed — still exists: ${ref.path}`,
      );
    }
  }
  for (const sub of YEAR_SUBCOLLECTIONS) {
  ...
  logStep(
    'Teardown verified — every registered doc is gone and year subtree is empty',
  );
}
...
describe(`E2E V4 — prod-safe full flow in sentinel year ${YEAR} (emulator default; LIVE_E2E=true for prod)`, () => {
  ...
  beforeAll(async () => {
    ...
    _tourneyRepo = new TourneyRepository();
  ...
  // ───────────────────────────────────────────────────────────────────────
  test('Full bracket → FF → games → 3 entries → ranking/ties → idempotency → email → CRUD → cleanup', async () => {
    ...
    // getAllTournamentDetails filters the regions list to the 4 bracket
    // quadrants (regionID 1-4); the Final Four / Championship
    // pseudo-regions (5-6) are excluded.
    expect(details.regions.length).toBe(4);
    ...
    const gamesById = new Map(
      details.allGames.map((g) => [Number(g.gameID), g]),
    );
    ...
    logStep(
      'Topology verified — 63 games, consistent nextGameID/nextGameSpot links',
    );
    ...
    logStep('Creating First Four game', {
      team1: FF_TEAM_A,
      team2: FF_TEAM_B,
      nextGame: FF_NEXT_GAME_ID,
      nextGameSpot: FF_NEXT_GAME_SPOT,
    });
    ...
    logStep('Resolving FF game — FF_TEAM_B wins', {
      winner: FF_TEAM_B,
      loser: FF_TEAM_A,
    });
    ...
    const game5AfterFF = gamesAfterFF.find(
      (g) => Number(g.gameID) === FF_NEXT_GAME_ID,
    );
    expect(Number(game5AfterFF.team2ID)).toBe(FF_TEAM_B);

    const allGamesAfterFF = await gameRepo.getAllTournamentDetails(YEAR);
    const ffGameDoc = allGamesAfterFF.allGames.find(
      (g) => Number(g.gameID) === FF_GAME_ID,
    );
    ...
    const ffGameUndone = gamesAfterFFUndo.find(
      (g) => Number(g.gameID) === FF_GAME_ID,
    );
    expect(ffGameUndone.winner == null).toBe(true);
    const game5AfterFFUndo = gamesAfterFFUndo.find(
      (g) => Number(g.gameID) === FF_NEXT_GAME_ID,
    );
    expect(
      game5AfterFFUndo.team2ID == null ||
        game5AfterFFUndo.team2ID === undefined,
    ).toBe(true);
    ...
    logStep(
      'FF undo verified — winner cleared, game 5 slot cleared, pick restored, hold set',
    );

    // ── FF-5. Write-time normalization against live game state ──────
    // Unresolved game: either FF team submitted → combined value (team1ID)
    const normUnresolved = await normalizeFirstFourPicks(
      [FF_TEAM_B, 116],
      YEAR,
    );
    expect(normUnresolved).toEqual([FF_TEAM_A, 116]);
    logStep(
      'normalizeFirstFourPicks (unresolved) verified — team B → combined team A',
    );

    // ── FF-5b. Pick-swap normalization must cover soft-deleted entries (#388) ──
    // Delete the entry, resolve the FF game, and verify the swap still
    // reached the deleted doc — an entry restored after the swap ran
    // must hold the winner, not the eliminated loser. Then repeat for
    // the undo path.
    logStep(
      'Soft-deleting FF entry, then resolving — swap must reach deleted docs (#388)',
    );
    ...
    const ffEntryDeletedSwapped = await gameRepo.getEntryById(ENTRY_FF, YEAR);
    expect(ffEntryDeletedSwapped.picks).toContain(FF_TEAM_B);
    expect(ffEntryDeletedSwapped.picks).not.toContain(FF_TEAM_A);
    await entryRepo.restoreEntry(ENTRY_FF, YEAR);
    const ffEntryRestored = await gameRepo.getEntryById(ENTRY_FF, YEAR);
    expect(ffEntryRestored.deletedAt == null).toBe(true);
    expect(ffEntryRestored.picks).toContain(FF_TEAM_B);
    logStep(
      'FF-5b resolve verified — deleted entry swapped to winner, restore sees it',
    );

    // Undo path: delete again, undo the game, restore — the pick must
    // have been normalized back to the combined value (team1).
    await entryRepo.deleteEntry(ENTRY_FF, YEAR);
    await undoTeamRecords(
      FF_TEAM_B,
      FF_TEAM_A,
    ...
    const ffEntryDeletedUndone = await gameRepo.getEntryById(ENTRY_FF, YEAR);
    expect(ffEntryDeletedUndone.picks).toContain(FF_TEAM_A);
    expect(ffEntryDeletedUndone.picks).not.toContain(FF_TEAM_B);
    await entryRepo.restoreEntry(ENTRY_FF, YEAR);
    logStep('FF-5b undo verified — deleted entry restored to combined pick');
    ...
    const ffGameReResolved = detailsAfterReResolve.allGames.find(
      (g) => Number(g.gameID) === FF_GAME_ID,
    );
    ...
    const game5AfterReResolve = gamesAfterReResolve.find(
      (g) => Number(g.gameID) === FF_NEXT_GAME_ID,
    );
    ...
    logStep(
      'FF re-resolution verified — pick on winner A, slot filled, hold released, stale loser pick normalizes to winner',
    );
    ...
    logStep(
      'FF cycle complete — resolve(B) → undo → re-resolve(A) → undo, picks combined again',
    );
    ...
    expect(
      game9AfterUndo.team2ID == null || game9AfterUndo.team2ID === undefined,
    ).toBe(true);
    logStep(
      'Undo verified — game 2 winner null, hold set, game 9 team2 cleared',
    );
    ...
    await updateTeamRecords(
      ...
      Number(game23.round),
      23,
      Number(game23.nextGameID),
      Number(game23.nextGameSpot),
      YEAR,
    );
    ...
    logStep('Creating entry Beta — identical picks (deliberate tie)', {
      id: ENTRY_B,
    });
    register(entryRef(ENTRY_B));
    await entryRepo.createEntry(
      ENTRY_B,
      `${EMAIL_PREFIX}-beta@example.com`,
      'E2E-V4 Beta',
      picksStrong,
      GROUP,
      'E2E-V4 Tester Beta',
    ...
    logStep('Creating entry Weak — eliminated unique picks', {
      id: ENTRY_C,
      picks: picksWeak,
    });
    ...
    const containingEntries = await gameRepo.getEntriesContainingTeams(
      YEAR,
      [67, 116],
    );
    ...
    logStep('getEntriesContainingTeams verified', {
      matchCount: containingEntries.length,
    });
    ...
    logStep('Running possibleRanking — leaderboard correctness', {
      group: GROUP,
    });
    ...
    logStep('possPoints decrease verified', {
      before: 182,
      after: alphaAfterElim.possPoints,
    });
    ...
    logStep(
      'Idempotency — re-running updatePossiblePoints over the same resolved games',
    );
    await updatePossiblePoints(YEAR);
    ...
    logStep(
      'Idempotency verified — points stable, records untouched, resolved games excluded from poll matching',
    );
    ...
    const game2Replayed = detailsAfterReplay.allGames.find(
      (g) => Number(g.gameID) === 2,
    );
    ...
    await updateTeamRecords(
      67,
      33,
      Number(game9Ready.round),
      9,
      Number(game9Ready.nextGameID),
      Number(game9Ready.nextGameSpot),
      YEAR,
    );
    logStep('Game 9 played — 67 beats 33 in R2');

    const gamesAfterR2 = await gameRepo.getActiveAndFutureGames(YEAR);
    const game13 = gamesAfterR2.find(
      (g) => Number(g.gameID) === Number(game9Ready.nextGameID),
    );
    ...
    if (APP_CONFIG.tournament.emailGroup === GROUP) {
      ...
      expect(
        svcUnsent[0].pickNames.some((n) => /^Team \d+$/.test(n) === false),
      ).toBe(true);
      ...
      logStep(
        'Service-level email skipped (EMAIL_GROUP not set to E2E-V4) — repo-level all-sent verified',
      );
    }

    // ── 21. Soft-delete entry Alpha and verify (#310) ──────────────
    logStep('Soft-deleting entry Alpha', { id: ENTRY_A });
    await entryRepo.deleteEntry(ENTRY_A, YEAR);

    // Document survives with a deletedAt stamp — not a hard delete.
    const afterSoftDelete = await gameRepo.getEntryById(ENTRY_A, YEAR);
    expect(afterSoftDelete).not.toBeNull();
    expect(typeof afterSoftDelete.deletedAt).toBe('string');

    // Hidden from every user-facing read path.
    expect(
      (await gameRepo.getEntriesForGroup(YEAR, GROUP)).map((e) => Number(e.id)),
    ).not.toContain(ENTRY_A);
    expect(
      (await gameRepo.getAllEntries(YEAR)).map((e) => Number(e.id)),
    ).not.toContain(ENTRY_A);
    expect(
      (await viewRepo.getGroupTeams(GROUP, YEAR)).map((e) => Number(e.id)),
    ).not.toContain(ENTRY_A);
    logStep(
      'Soft delete verified — entry hidden from group/all-entries/groupTeams reads',
    );

    // A hard delete is refused until the entry has been soft-deleted —
    // exercise the guard against a still-live entry (Beta).
    await expect(entryRepo.purgeEntry(ENTRY_B, YEAR)).rejects.toThrow();
    logStep('purgeEntry guard verified — refuses a not-yet-soft-deleted entry');

    // Restore brings it back everywhere.
    await entryRepo.restoreEntry(ENTRY_A, YEAR);
    const afterRestore = await gameRepo.getEntryById(ENTRY_A, YEAR);
    expect(afterRestore.deletedAt).toBeUndefined();
    expect(
      (await gameRepo.getEntriesForGroup(YEAR, GROUP)).map((e) => Number(e.id)),
    ).toContain(ENTRY_A);
    logStep('restoreEntry verified — entry visible again');

    // Soft-delete once more, then permanently purge.
    await entryRepo.deleteEntry(ENTRY_A, YEAR);
    await entryRepo.purgeEntry(ENTRY_A, YEAR);
    const afterPurge = await gameRepo.getEntryById(ENTRY_A, YEAR);
    expect(afterPurge == null).toBe(true);
    logStep('Entry purge verified (Beta and Weak removed by teardown)');
    ...
    const fetchedConf =
      await conferenceRepo.getConferenceBySlug(CRUD_CONF_SLUG);
    ...
    await teamRepo.insertSchool({
      ...
      conferenceHistory: [
        { confID: CRUD_CONF_SLUG, startYear: 2020, endYear: null },
      ],
    });
...
describe('E2E V4 — getAllYearsForGroup Filter.or() over groups + legacy group', () => {
  ...
  const cleanup = async () => {
    for (const y of [NEW_YEAR, LEGACY_YEAR]) {
      ...
      try {
        await db.collection('tournaments').doc(String(y)).delete();
      } catch {
        /* best-effort */
      }
    }
  ...
  beforeAll(async () => {
    ...
    await db
      .collection('tournaments')
      .doc(String(LEGACY_YEAR))
      .set({ year: LEGACY_YEAR });
    ...
    await db
      ...
      .set({
        id: 1,
        email: 'e2ev4-filteror@example.com',
        teamName: 'E2E-V4 FilterOr New',
        picks: [],
        groups: [NEW_GROUP],
        person: 'E2E-V4 FilterOr',
        totalPoints: 0,
        possPoints: 0,
      });
    ...
    await db
      ...
      .set({
        id: 1,
        email: 'e2ev4-filteror@example.com',
        teamName: 'E2E-V4 FilterOr Legacy',
        picks: [],
        group: LEGACY_GROUP,
        person: 'E2E-V4 FilterOr',
        totalPoints: 0,
  ...
  test('a group with no entries matches neither reserved year', async () => {
    ...
    expect(
      years.some((y) => y.year === NEW_YEAR || y.year === LEGACY_YEAR),
    ).toBe(false);
  });
});
