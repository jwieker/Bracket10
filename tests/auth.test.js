...
vi.mock('google-auth-library', () => {
  return {
    OAuth2Client: vi
      .fn()
      .mockImplementation(function (clientId, clientSecret, redirectUri) {
        return {
...
describe('auth.js utility functions', () => {
  ...
  test('_getRedirectUriForTests prefers REDIRECT_URI when set, regardless of NODE_ENV or APP_HOST', () => {
    process.env.REDIRECT_URI =
      'https://tunnel.example.com/auth/google/callback';
    process.env.NODE_ENV = 'development';
    process.env.APP_HOST = 'ignored.example.com';
    expect(_getRedirectUriForTests()).toBe(
      'https://tunnel.example.com/auth/google/callback',
    );
  });
  ...
  test('getOAuthClient creates a client lazily and caches it', () => {
    ...
    expect(client1.redirectUri).toBe(
      'http://localhost:8080/auth/google/callback',
    );
