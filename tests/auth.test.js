import { getGoogleClientId, getOAuthClient, _getRedirectUriForTests, getAuthUrl, isAdminEmail } from "../src/config/auth.js";
...
describe("auth.js utility functions", () => {
  ...
  test("_getRedirectUriForTests prefers REDIRECT_URI when set, regardless of NODE_ENV or APP_HOST", () => {
    process.env.REDIRECT_URI = "https://tunnel.example.com/auth/google/callback";
    process.env.NODE_ENV = "development";
    process.env.APP_HOST = "ignored.example.com";
    expect(_getRedirectUriForTests()).toBe("https://tunnel.example.com/auth/google/callback");
  });

  test("_getRedirectUriForTests returns production callback using APP_HOST when NODE_ENV is production", () => {
    process.env.NODE_ENV = "production";
    process.env.APP_HOST = "example.com";
    expect(_getRedirectUriForTests()).toBe("https://example.com/auth/google/callback");
  });

  test("_getRedirectUriForTests falls back to localhost in production when APP_HOST is unset", () => {
    process.env.NODE_ENV = "production";
    delete process.env.APP_HOST;
    process.env.PORT = "8080";
    expect(_getRedirectUriForTests()).toBe("http://localhost:8080/auth/google/callback");
  });

  test("_getRedirectUriForTests returns localhost callback using PORT if NODE_ENV is not production", () => {
    process.env.NODE_ENV = "development";
    process.env.PORT = "3000";
    expect(_getRedirectUriForTests()).toBe("http://localhost:3000/auth/google/callback");
