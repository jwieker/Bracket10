...
describe('registerFailedAttempt', () => {
  ...
  test('starts a window at count 1 when no doc exists', async () => {
    const now = 1_000_000;
    const result = await registerFailedAttempt('login:1.2.3.4', 10_000, 5, now);
  ...
  test('increments within the same window and keeps resetTime', async () => {
    const now = 1_000_000;
    await registerFailedAttempt('k', 10_000, 5, now);
    const second = await registerFailedAttempt('k', 10_000, 5, now + 5_000);
  ...
  test('resets to count 1 once the window has elapsed', async () => {
    const now = 1_000_000;
    await registerFailedAttempt('k', 10_000, 5, now);
    const later = await registerFailedAttempt('k', 10_000, 5, now + 10_001);
  ...
  test('skips the write once count has reached max (no contention under flood)', async () => {
    const now = 1_000_000;
    await registerFailedAttempt('k', 10_000, 2, now);          // count 1, writes
    await registerFailedAttempt('k', 10_000, 2, now + 1_000);  // count 2 (== max), writes
    expect(stored.count).toBe(2);

    // Already at max: returns a blocking count but must NOT write.
    const blocked = await registerFailedAttempt('k', 10_000, 2, now + 2_000);
