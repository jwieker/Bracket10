...
const ESPN_GAMES_SPEC = [
  '1-1-1-116',
  '1-1-16-28', // game 1: Gonzaga (seed 1) vs Duke (seed 16)
  '1-2-8-73',
  '1-2-9-55', // game 2: Kansas (seed 8) vs Arizona (seed 9)
];
...
describe('E2E V3 — ESPN poll, cache invalidation, conference history, validation', () => {
  ...
  describe('ESPN poll flow (year 9997)', () => {
    ...
    test('single game resolved via poll — updates DB and entry points', async () => {
      ...
      const summary = await runEspnPoll(ESPN_YEAR, {
        dryRun: false,
        dateStr: '20260607',
      });
      ...
      try {
        ...
        logStep('Single-game poll verified', {
    ...
    test('unmapped team name is logged and poll continues for mapped games', async () => {
      ...
      expect(summary.updated).toBe(1);
      logStep('Unmapped team handling verified', {
  ...
  describe('cache invalidation (live Firestore reads/writes)', () => {
    afterAll(async () => {
      try {
        await teamRepo.deleteSchool(CACHE_SCHOOL_SID);
      } catch {
        /* best-effort */
      }
      clearAllCache();
  ...
  describe('conference history migrations (global school/conference docs)', () => {
    beforeAll(async () => {
      ...
      logStep('Test conferences inserted', {
        slugA: CONF_SLUG_A,
        slugB: CONF_SLUG_B,
      });
    }, 30000);
    ...
    afterAll(async () => {
      ...
      for (const sid of [CONF_SCHOOL_SID_A, CONF_SCHOOL_SID_B]) {
        try {
          await teamRepo.deleteSchool(sid);
        } catch {
          /* best-effort */
        }
      }
      ...
      try {
        await db.collection('conferences').doc(CONF_SLUG_A).delete();
      } catch {
        /* best-effort */
      }
      try {
        await db.collection('conferences').doc(CONF_SLUG_B).delete();
      } catch {
        /* best-effort */
      }
    ...
    test('insertSchool stores conferenceHistory as provided', async () => {
      await teamRepo.insertSchool({
        ...
        confID: CONF_SLUG_A,
        conferenceHistory: [
          { confID: CONF_SLUG_A, startYear: null, endYear: null },
        ],
      });
      ...
      logStep('School A conference history bootstrapped', {
        sid: CONF_SCHOOL_SID_A,
      });
    });
    ...
    test('updateSchoolConferenceHistory replaces the full history array', async () => {
      ...
      await teamRepo.updateSchoolConferenceHistory(
        CONF_SCHOOL_SID_A,
        newHistory,
      );
      ...
      expect(fetched.conferenceHistory[0]).toMatchObject({
        confID: CONF_SLUG_A,
        startYear: 2010,
        endYear: 2022,
      });
      expect(fetched.conferenceHistory[1]).toMatchObject({
        confID: CONF_SLUG_B,
        startYear: 2022,
        endYear: null,
      });
    ...
    test('full realignment flow — school moves to new conference', async () => {
      await teamRepo.insertSchool({
        ...
        confID: CONF_SLUG_A,
        conferenceHistory: [
          { confID: CONF_SLUG_A, startYear: 2020, endYear: null },
        ],
      });
...
async function preCleanEspnYear(year, gameRepo, tourneyRepo, entryRepo) {
  ...
  try {
    ...
    console.log(
      `[E2E-V3] Deleted ${e2e.length} e2e-v3 entries for year ${year}`,
    );
  } catch (err) {
