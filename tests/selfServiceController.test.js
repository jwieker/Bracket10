import { myEntryLookup, myEntryVerify, myEntryView, myEntryUpdate, myBrackets, userEntryView, userEntryUpdate } from '../src/controllers/selfServiceController.js';
...
import { getEntriesForUser, getGroupRegistrationData, calculateMaxPossiblePoints, normalizeFirstFourPicks, normalizeAndValidateEntryPicks } from '../src/services/index.js';
...
describe('myEntryUpdate', () => {
  ...
  test('persists exactly the picks returned by the pipeline (FF winner lands in the entry)', async () => {
    ...
    const res = mockRes();
    await myEntryUpdate(req, res);
    expect(normalizeAndValidateEntryPicks).toHaveBeenCalledWith(tenPickIds, '2024', 'House');
    expect(gameRepository.updateEntry).toHaveBeenCalledWith(expect.objectContaining({
