...
const GAMES_SPEC_2022 = [
    ...
    "1-5-6-78",   "1-5-11-39",
    "1-6-3-23",   "1-6-14-213",
    "1-7-7-157",  "1-7-10-123",
    "1-8-2-46",   "1-8-15-223",
    "2-16-1-116", "2-16-16-174",  // R1: 116 vs 174  ← played in beforeAll
    "2-17-8-126", "2-17-9-82",
    "2-18-5-1",   "2-18-12-99",
    "2-19-4-42",  "2-19-13-290",
    "2-20-6-41",  "2-20-11-35",
    "2-21-3-79",  "2-21-14-327",
    "2-22-7-17",  "2-22-10-137",
    "2-23-2-28",  "2-23-15-293",
    "3-31-1-55",  "3-31-16-204",
    "3-32-8-8",   "3-32-9-77",
    "3-33-5-253", "3-33-12-343",
    "3-34-4-13",  "3-34-13-243",
    "3-35-6-127", "3-35-11-16",
    "3-36-3-52",  "3-36-14-337",
    "3-37-7-21",  "3-37-10-143",
    "3-38-2-10",  "3-38-15-185",
    "4-46-1-73",  "4-46-16-272",
    "4-47-8-132", "4-47-9-3",
    "4-48-5-15",  "4-48-12-146",
    "4-49-4-7",   "4-49-13-282",
    "4-50-6-47",  "4-50-11-72",
    "4-51-3-25",  "4-51-14-237",
    "4-52-7-63",  "4-52-10-32",
    "4-53-2-43",  "4-53-15-95",
];
...
describe("LIVE — Firestore Targeted Updates (requires LIVE_E2E=true)", () => {
    ...
    const logStep = (msg, extra = {}) => {
        const str = JSON.stringify(extra, (_, v) => (typeof v === "bigint" ? v.toString() : v));
        // eslint-disable-next-line no-console
        console.log(`[TARGETED][${new Date().toISOString()}] ${msg}${str !== "{}" ? ` | ${str}` : ""}`);
    };
    ...
    beforeAll(async () => {
        ...
        await entryRepo.createEntry(
            ENTRY_A_ID, "a@targeted-test.com", "Entry A", PICKS_A,
            GROUP, "Tester A", new Date(), year, 320
        );
        await entryRepo.createEntry(
            ENTRY_B_ID, "b@targeted-test.com", "Entry B", PICKS_B,
            GROUP, "Tester B", new Date(), year, 320
        );
        ...
        await updateTeamRecords(
            116, 174,
            Number(game16.round), 16,
            Number(game16.nextGameID), Number(game16.nextGameSpot),
            year
        );
    ...
    test("Change 1 — getActiveAndFutureGames excludes games that already have a winner", async () => {
        ...
        logStep("Change 1 verified — decided game absent, undecided games present", {
    ...
    test("Change 3a — updatePointsForAffectedEntries correctly updates points for affected entry", async () => {
        ...
        await updateTeamRecords(
            67, 307,
            Number(game1.round), 1,
            Number(game1.nextGameID), Number(game1.nextGameSpot),
            year
        );
    ...
    test("Change 3b — updatePointsForAffectedEntries does not modify entries without the affected teams", async () => {
        ...
        logStep("Change 3b verified — Entry B totalPoints remains 0 (unaffected by targeted update)");
    });
});
...
async function cleanTargetedYear(year, entryRepo, gameRepo, tourneyRepo) {
    ...
    try {
        ...
        for (const e of toDelete) await entryRepo.deleteEntry(Number(e.id), year);
        // eslint-disable-next-line no-console
